...
class pooling:
    ...
    def getSourceModule(self):
        ...
        __global__ void pool_ave_3d_res(float* __restrict__ M, float* __restrict__ P, 
                                       const int wM, const int hM,
                                       const int stride, const int window,
                                       const int wP, const int hP){
          // window must have the same width and height
    ...
    def ave_naive_3d_res(self, M, stride, window):
        start = cuda.Event()
        end = cuda.Event()
        func = self.mod.get_function("pool_ave_3d_res")
...
if __name__ == "__main__":
    ...
    for i in size:
        ...
        P_res, t_res = cuda_model.ave_naive_3d(M_1, stride, window_size)

        t_single = 0
        P_2D = np.zeros(shape=(N, int(i / 2), int(i / 2)))      # convert 3D matrix into multiple 2D matrices
        for k in range(N):
            P_2D[k, :], t_single_i = cuda_model.ave_naive(M_1[k, :], stride, window_size) # iteratively call 2D pooling
            t_single += t_single_i

        try:
            # print("Checkpoint: Do python and gpu convolution match? Checking...")
            assert ((P_res == P_py).all())
            print("match")
    ...
    plt.plot(size, t_naive_list, 'b', label="3D pool")
    plt.plot(size, t_py_list, 'g', label="numpy.mean")
    plt.plot(size, t_single_list, 'r', label="naive 2D iteration")
    plt.plot(size, t_res_list, 'y', label="3D+res")
