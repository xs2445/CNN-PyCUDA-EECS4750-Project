        ...
        // output P size = Mh*Nw
            ...
            for(int i=0; i<strideM;i++){
                ...
                if (i*blocksize+tx<Mw && row <Mh){
                    MS[ty][tx] = M[row*Mw+i*blocksize+tx];      // load blocks of M into shared memory
                }
                else{
                    MS[ty][tx] = 0.0f;       //padding block elements that exceeds M's margin with 0
                }
...
if __name__ == "__main__":
    ...
    for i in size:
        ...
        P_naive_res, t_naive_res = cuda_model.multi_naive_restrict(M, N, M_w, M_h, N_w)
        P_shared, t_shared = cuda_model.multi_tile(M, N, M_w, M_h, N_w)
        P_shared_res, t_shared_res = cuda_model.multi_naive_restrict(M, N, M_w, M_h, N_w)
        P_py, t_py = pytest(M, N)

        t_naive_list.append(t_naive)
        t_naive_res_list.append(t_naive_res)
        t_shared_list.append(t_shared)
        t_shared_res_list.append(t_shared_res)
        # dd
        t_py_list.append(t_py)

        try:
            # print("Checkpoint: Do python and gpu convolution match? Checking...")
            assert ((P_shared_res == P_py).all())
            print("match, result:")
    ...
    plt.plot(size, t_naive_res_list, label="naive+res ")
    plt.plot(size, t_shared_list, 'g', label="shared memory")
    plt.plot(size, t_shared_res_list, label="shared memory+res")
    # plt.plot(size, t_py_list, 'r', label="numpy.dot")
    plt.legend()
    plt.show()
    fig1.savefig('pycuda_multiplication.png')
